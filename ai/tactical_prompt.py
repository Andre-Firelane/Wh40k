...
TACTICAL_SYSTEM_PROMPT = (
    ...
    "  Riled up (the Orks army rule Waaagh!) changes that: while a unit is riled up, an Advance "
    "option's own description will say the unit can still declare a charge, and its ranged weapons "
    "have [ASSAULT]. For a melee-oriented unit with no strong ranged weapon it would rather keep using, "
