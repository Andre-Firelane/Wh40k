...
class UnitProfile:
    ...
    bloody_handed = False  # the Avatar of Khaine's own aura: friendly AELDARI units within 6" add 1 to Advance and Charge rolls. Read through game/roll_bonus.py, which is the one place the two roll sites ask - see game/bloody_handed.py
    ...
    headwoppas_killchoppa = False  # War Horde Enhancement: +1 AP on the BEARER's melee attacks once its unit charged this turn - game/fight.py's chain and _melee_attack_key(); see game/enh_headwoppas_killchoppa.py
    ...
    # --- Canoptek Court Enhancements (game/enhancements.py registers all four) ---
    dimensional_sanctum = False  # CRYPTEK bearer: "Models in the bearer's unit have the Infiltrators ability" - a unit-level grant read by game/squad.py's squad_has_infiltrators(); see game/enh_dimensional_sanctum.py
    hyperphasic_fulcrum = False  # CRYPTEK bearer: while leading a unit wholly within the Power Matrix, that unit re-rolls Wound rolls of 1 - both attack steps' automatic 1s chains; see game/enh_hyperphasic_fulcrum.py
    autodivinator = False  # CRYPTEK bearer: each time the opponent gains a CP from an ABILITY, roll a D6 - on 2+ gain 1CP too. A CommandPointManager.on_cp_gained listener; see game/enh_autodivinator.py
    metalodermal_tesla_weave = False  # CRYPTEK bearer: once per phase, when an enemy unit charges the bearer's unit, D6 -> 2-5 D3 / 6 flat 3 mortal wounds on the charger. A charge_declaration_reactions entry; see game/enh_metalodermal_tesla_weave.py
    # --- Hypercrypt Legion Enhancements (game/enhancements.py registers all four) ---
    dimensional_overseer = False  # NECRONS bearer: while it is on the battlefield or in Strategic Reserves, Hyperphasing may select one more unit - read by game/hypercrypt_hyperphasing.py's limit(); see game/enh_dimensional_overseer.py
    arisen_tyrant = False  # NECRONS bearer: its unit re-rolls Hit rolls of 1, and the WHOLE Hit roll instead if it was set up on the battlefield this turn - a game/reroll_scope.py ones-or-whole source at the Power Matrix's five seams; see game/enh_arisen_tyrant.py
    hyperspatial_transfer_node = False  # NECRONS bearer: its unit Advances without a roll, +6" Move until the end of the phase - MovementController.start_run()'s no-roll branch; see game/enh_hyperspatial_transfer_node.py
    osteoclave_fulcrum = False  # NECRONS bearer: models in its unit have Deep Strike - written onto the models' profiles at the start of Declare Battle Formations, Student of Kauyon's seam; see game/enh_osteoclave_fulcrum.py
