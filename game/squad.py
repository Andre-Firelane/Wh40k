...
class Squad:
    ...
    def __init__(self, name, models, owner="Player 1", points=None):
        ...
        self.warpath_active = False  # Kill Rig's Warpath (2026-09 codex): this unit's melee attacks have [LETHAL HITS] and [PSYCHIC] until the end of the phase - see game/warpath.py; cleared with the other per-phase grants in main.py
        self.fix_dat_armour_up_used = False  # the Big Mek in Mega Armour's Fix Dat Armour Up: the once-per-battle-per-unit spend - see game/fix_dat_armour_up.py; saved
        self.makari_used = False  # Ghazghkull's Makari, Hoist Dat Banner!: the once-per-battle-per-ARMY spend, on the unit that used it - see game/makari.py; saved
        self.da_boss_round = None  # the battle round Da Boss last paid this Warlord's player a CP - see game/da_boss.py; saved, so a mid-round load does not pay twice
        self.weirdboy_warpath_active = False  # the Weirdboy's Warpath (2026-09 codex): this unit's melee attacks can re-roll wound rolls of 1 and have [PSYCHIC] until the end of the phase - see game/weirdboy_warpath.py; cleared with the Kill Rig's in main.py's per-phase block
        self.da_jump_deep_strike = False  # the Weirdboy's Da Jump (2026-09 codex): "This unit has Deep Strike" - a unit-level grant with no end, read by game/ingress.py's _has_deep_strike(); see game/da_jump.py
        self.da_jump_round = None  # ...and the battle round this unit used Da Jump in - once per battle round per ARMY, see game/da_jump.py; saved
        self.beastscent_active = False  # Kill Rig's Beastscent (2026-09 codex), on the DISEMBARKED unit: its attacks against MONSTER/VEHICLE units have +1 to wound until the end of the turn - see game/beastscent.py; cleared by main.py's end-of-turn sweep over every unit
        ...
        self.eternity_gate_charge_locked = False  # the Monolith's Eternity Gate: "That unit cannot make a charge move this turn" - its OWN lock rather than charge_locked_until_end_of_turn, because Hypercrypt Legion's Dimensional Corridor lifts THIS lock and no other (a unit that also bought Cosmic Precision stays locked). Set by game/eternity_gate.py, read by game/charge.py, cleared on every squad at the end of every turn
        self.eternity_gate_bearer_started_on_board = False  # ...and whether the Monolith whose gate this unit came through had started the turn on the battlefield - Dimensional Corridor's TARGET asks it; same lifetime
